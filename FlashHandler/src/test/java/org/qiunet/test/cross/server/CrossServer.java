	...
	public static void main(String[] args) {
		...
		ClassScanner.getInstance(ScannerType.SERVER)
			...
			BootstrapServer.createBootstrap(hook)
				.listener(ServerBootStrapParam.newBuild("跨服服务", Constants.CROSS_SERVER_PORT).setStartupContext(IStartupContext.DEFAULT_CROSS_START_CONTEXT).setProtocolHeaderType(ProtocolHeaderType.cross).build())
				.listener(ServerBootStrapParam.newBuild("跨服节点", Constants.CROSS_NODE_PORT).setStartupContext(IStartupContext.DEFAULT_CROSS_NODE_START_CONTEXT).setProtocolHeaderType(ProtocolHeaderType.node).build())
				.await();
	}
