	...
	public static void main(String[] args) {
		...
		ClassScanner.getInstance(ScannerType.SERVER)
			...
			BootstrapServer.createBootstrap(hook)
				.listener(ServerBootStrapParam.newBuild("测试服务", Constants.LOGIC_SERVER_PORT).setStartupContext(IStartupContext.SERVER_STARTUP_CONTEXT).setProtocolHeaderType(ProtocolHeaderType.server).build())
				.listener(ServerBootStrapParam.newBuild("节点服务",Constants.LOGIC_NODE_PORT).setStartupContext(IStartupContext.DEFAULT_CROSS_NODE_START_CONTEXT).setProtocolHeaderType(ProtocolHeaderType.node).build())
				.await();
	}
