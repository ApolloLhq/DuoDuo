	...
	public void run() {
		try {
			...
			b.group(GROUP)
					...
					.childAttr(ServerConstants.PROTOCOL_HEADER_ADAPTER, param.getProtocolHeaderType())
					.childHandler(new ChannelInitializer<UkcpChannel>() {
						@Override
						public void initChannel(UkcpChannel ch) throws Exception {
							ChannelPipeline p = ch.pipeline();
							p.addLast("KcpSocketEncoder", new KcpSocketEncoder())
							.addLast("KcpSocketDecoder", new KcpSocketDecoder(param.getMaxReceivedLength(), param.isEncryption()))
